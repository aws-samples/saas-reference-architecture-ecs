...
# service-info.json is generated by build-application.sh (single source of truth)
# provision-tenant.sh also generates it independently in CodeBuild

# Create S3 Bucket and upload provision source
...
# npx --yes cdk deploy shared-infra-stack --require-approval=any-change 
...
npx --yes cdk deploy shared-infra-stack --require-approval never --concurrency 10 --asset-parallelism true

# Deploy tenant infra stacks (ECS cluster, Cognito, namespace)
CDK_PARAM_TENANT_ID=basic CDK_PARAM_TIER=BASIC npx --yes cdk deploy tenant-template-stack-basic --exclusively --require-approval never --concurrency 10 --asset-parallelism true
CDK_PARAM_TENANT_ID=advanced CDK_PARAM_TIER=ADVANCED CDK_ADV_CLUSTER=INACTIVE npx --yes cdk deploy tenant-template-stack-advanced --exclusively --require-approval never --concurrency 10 --asset-parallelism true

# Deploy tenant service stacks (ECS services, DynamoDB tables, ALB rules)
CDK_PARAM_TENANT_ID=basic CDK_PARAM_TIER=BASIC npx --yes cdk deploy tenant-service-stack-basic --exclusively --require-approval never --concurrency 10 --asset-parallelism true
# 