...
class AuthConfigurationService {
  ...
  async setTenantConfig(tenantName: string): Promise<ConfigParams> {
    try {
      ...
      // Store in localStorage like Angular version
  ...
  cleanChromeExtensionData(): void {
    if (process.env.NODE_ENV === 'development') {
      ...
      console.log('Chrome extension data cleaned from storage');
    }
  }
  ...
  getUserPoolId(): string | null {
    return localStorage.getItem('userPoolId');
  }

  getAppClientId(): string | null {
    return localStorage.getItem('appClientId');
  }
}
