...
export class SharedInfraStack extends cdk.Stack {
  ...
  constructor (scope: Construct, id: string, props: SharedInfraProps) {
    ...
    // Replaces per-stack AwsCustomResource lambdas (DynamoDB mapping + MySQL schema invoke)
    const tenantCustomResourceFn = new lambda.Function(this, 'TenantCustomResourceFn', {
      ...
      description: 'Shared custom resource handler for tenant lifecycle (DynamoDB mapping, MySQL schema)',
    });

    // Grant DynamoDB access for tenant mapping operations
    this.tenantMappingTable.grantReadWriteData(tenantCustomResourceFn);

    // Grant Lambda invoke for MySQL schema provisioning (conditional)
    tenantCustomResourceFn.addToRolePolicy(new iam.PolicyStatement({
      actions: ['lambda:InvokeFunction'],
      resources: ['*'], // Schema lambda ARN is dynamic per deployment
    }));
