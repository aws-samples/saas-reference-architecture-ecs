...
import { EcsService } from './services-parallel';
...
export class TenantTemplateStack extends cdk.Stack {
  ...
  constructor (scope: Construct, id: string, props: TenantTemplateStackProps) {
    ...
    if( 'advanced' !== props.tier.toLocaleLowerCase() || 'ACTIVE' === props.advancedCluster) {
      ...
      // 서비스 의존성 정의 - 필요한 경우 여기에 의존성을 추가
      const serviceDependencies: ServiceDependency[] = [
        // 예: { name: "service1", dependsOn: ["service2", "service3"] }
        // 현재는 의존성이 없는 것으로 가정
      ];

      // 서비스 생성 및 의존성 관리를 위한 맵
      const serviceMap = new Map<string, EcsService>();
      ...
      containerInfo.forEach((info) => {
        ...
        // 서비스 맵에 추가
        serviceMap.set(info.name, ecsService);
      });

      // 의존성 설정
      serviceDependencies.forEach(dependency => {
        const service = serviceMap.get(dependency.name);
        if (service) {
          dependency.dependsOn.forEach(dependsOnName => {
            const dependsOnService = serviceMap.get(dependsOnName);
            if (dependsOnService) {
              service.node.addDependency(dependsOnService);
            }
          });
        }
      });

      // Reverse Proxy 서비스 생성 (다른 모든 서비스에 의존)
      if (isRProxy) {
